...
class Admin::ClassifierController < ApplicationController
  ...
  def index
    ...
    test_labels = []
    test_documents = []
    @split_point = Article.count*2/3.round
    @total = Article.count
    Article.all.each_with_index do |article, index|
      if index < @split_point
        # Training set:
        labels << article.contains_hatespeech
        documents << %w[#{article.text}]
      else
        # Testing set:
        test_labels = article.contains_hatespeech
        test_documents = %w[#{article.text}]
      end
    end
